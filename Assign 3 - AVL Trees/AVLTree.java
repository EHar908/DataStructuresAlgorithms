...
public class AVLTree<E extends Comparable<? super E>> {
    ...
    public E deleteMin() {
        AvlNode smallestGuy = findSmallest(root, null);
        //balance(root);
        return smallestGuy.value;
        //return (E) findSmallest(root, null);
    ...
    public AvlNode findSmallest(AvlNode node, AvlNode parent){
        AvlNode smallestNode;
        if(node.left != null){
            smallestNode = findSmallest(node.left, node);
        }
        else{
            smallestNode = node;
            if(node != root){ //The root-case when parent = null.
    ...
    public void printTree(String label) {
        System.out.println(traverseAVL(root, 0));
    }

    public String traverseAVL(AvlNode node, int depth){
        String result = ""; //Designate the string we're going to return.
        if(node == null) {
            return "";
        }
        result += traverseAVL(node.right, depth+1);
        for(int i = 0; i < depth; i++){
            result += "   ";
        }
        result = result + node.value + "(" + node.height + ")";
        result += "\n";
        result += traverseAVL(node.left, depth+1);
        return result;
    }
